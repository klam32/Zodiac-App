...
const PaymentReportModal: React.FC<PaymentReportModalProps> = ({ user, onClose }) => {
  ...
  const handleFileChange = async (file: File) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      toast.error(t('payment.report.invalidType', 'Chỉ hỗ trợ ảnh JPG, PNG hoặc WEBP.'));
      return;
    }

    const maxSize = 5 * 1024 * 1024;
    if (file.size > maxSize) {
      toast.error(t('payment.report.invalidSize', 'Ảnh minh chứng không được vượt quá 5MB.'));
      return;
    }
    ...
    try {
      ...
      setIsUploaded(true);
      toast.success(t('payment.report.uploadSuccess', 'Tải lên ảnh minh chứng thành công!'));
    } catch (err: any) {
      toast.error(err.message || t('payment.report.uploadError', 'Lỗi tải lên ảnh minh chứng.'));
  ...
  const handleSubmit = async () => {
    if (!title.trim()) {
      toast.error(t('payment.report.titleRequired', 'Vui lòng nhập tiêu đề sự cố.'));
      return;
    }
    if (!description.trim()) {
      toast.error(t('payment.report.descriptionRequired', 'Vui lòng nhập mô tả chi tiết.'));
      return;
    }
    if (description.trim().length < 10) {
      toast.error(t('payment.report.descriptionMinLength', 'Mô tả sự cố phải dài ít nhất 10 ký tự.'));
      return;
    }
    if (uploading) {
      toast.error(t('payment.report.waitUpload', 'Vui lòng đợi ảnh minh chứng tải lên xong.'));
      return;
    }

    setIsSubmitting(true);
    const loadingToast = toast.loading(t('payment.report.sending', 'Đang gửi báo cáo sự cố...'));
    try {
      ...
      toast.success(t('payment.report.sendSuccess', 'Gửi báo cáo thành công! Ban quản trị sẽ kiểm tra sớm và gửi email phản hồi.'));
      onClose();
    } catch (err: any) {
      toast.error(err.message || t('payment.report.sendError', 'Không thể gửi báo cáo'));
    } finally {
    ...
    <div className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
      ...
      <div className="bg-[#1e1d1b] border border-stone-800 rounded-[28px] max-w-xl w-[92%] md:w-full relative shadow-2xl flex flex-col max-h-[85vh] overflow-hidden text-stone-200">
        ...
        <div className="p-6 pb-4 flex justify-between items-start border-b border-stone-800/60 flex-shrink-0">
          <div>
            <h3 className="text-xl font-bold text-amber-500">{t('payment.report.modalTitle', 'Báo cáo sự cố & thanh toán')}</h3>
            <p className="text-stone-400 text-xs mt-1">{t('payment.report.modalSubtitle', 'Cung cấp chi tiết sự cố để ban quản trị xử lý nhanh nhất.')}</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-4 pr-4 custom-modal-scroll text-sm">
          ...
          <div className="bg-stone-950/50 border border-stone-800/80 rounded-2xl p-4 grid grid-cols-3 gap-3 text-xs">
            <div>
              <span className="text-stone-500 block uppercase font-bold tracking-wider text-[9px] mb-0.5">{t('payment.report.account', 'Tài khoản')}</span>
              <span className="text-stone-200 font-semibold truncate block" title={user?.username}>{user?.username || 'N/A'}</span>
            </div>
            <div>
              <span className="text-stone-500 block uppercase font-bold tracking-wider text-[9px] mb-0.5">{t('common.email', 'Email')}</span>
              <span className="text-stone-200 font-semibold truncate block" title={user?.email}>{user?.email || 'N/A'}</span>
            </div>
            <div>
              <span className="text-stone-500 block uppercase font-bold tracking-wider text-[9px] mb-0.5">{t('payment.report.balance', 'Số dư')}</span>
          ...
          <div>
            <label className="text-xs font-semibold text-stone-300 block mb-1.5">
              {t('payment.report.issueTitle', 'Tiêu đề sự cố')} <span className="text-amber-500">*</span>
            </label>
            <input 
              type="text"
              placeholder={t('payment.report.titlePlaceholder', 'Ví dụ: Đã chuyển khoản nhưng chưa nhận token...')}
          ...
          <div>
            <label className="text-xs font-semibold text-stone-300 block mb-1.5">
              {t('payment.report.issueType', 'Loại sự cố / Yêu cầu')} <span className="text-amber-500">*</span>
            </label>
            <select
              ...
              disabled={isSubmitting}
            >
              <option value="payment_not_received">{t('payment.report.typeNotReceived', 'Đã thanh toán nhưng chưa nhận token')}</option>
              <option value="wrong_token_amount">{t('payment.report.typeWrongAmount', 'Sai số token nhận được')}</option>
              <option value="payment_failed">{t('payment.report.typeFailed', 'Thanh toán thất bại / Lỗi cổng thanh toán')}</option>
              <option value="duplicate_payment">{t('payment.report.typeDuplicate', 'Thanh toán bị trùng lặp giao dịch')}</option>
              <option value="other">{t('payment.report.typeOther', 'Vấn đề khác')}</option>
            </select>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-semibold text-stone-300 block mb-1.5">
                {t('payment.report.invoiceCode', 'Mã hóa đơn (nếu có)')}
              </label>
              <input 
                type="text"
                placeholder={t('payment.report.invoicePlaceholder', 'Ví dụ: #71')}
            ...
            <div>
              <label className="text-xs font-semibold text-stone-300 block mb-1.5">
                {t('payment.report.transactionCode', 'Mã giao dịch / Transaction')}
              </label>
              <input 
                type="text"
                placeholder={t('payment.report.transactionPlaceholder', 'Ví dụ: FT260529...')}
                className="w-full bg-[#2a2926] border border-stone-700 rounded-xl px-4 py-3 text-sm text-stone-100 placeholder:text-stone-400 focus:outline-none focus:border-amber-500 transition-all"
          ...
          <div>
            <label className="text-xs font-semibold text-stone-300 block mb-1.5">
              {t('payment.report.descriptionLabel', 'Mô tả chi tiết sự cố')} <span className="text-amber-500">*</span>
            </label>
            <textarea 
              rows={4}
              placeholder={t('payment.report.descriptionPlaceholder', 'Vui lòng cung cấp thêm thông tin chi tiết (thời gian, hình thức thanh toán, số tiền chuyển khoản...)')}
            ...
            <span className="text-[10px] text-stone-500 block mt-1 px-1">
              {t('payment.report.descriptionLengthNote', 'Tối thiểu 10 ký tự. Đã nhập: {{count}} ký tự.', { count: description.trim().length })}
            </span>
          </div>
          ...
          <div>
            <label className="text-xs font-semibold text-stone-300 block mb-0.5">
              {t('payment.report.evidenceLabel', 'Ảnh minh chứng (nếu có)')}
            </label>
            <p className="text-[11px] text-stone-400 mb-2">
              {t('payment.report.evidenceSubtitle', 'Bạn có thể tải lên ảnh chụp màn hình giao dịch hoặc biên lai thanh toán.')}
            </p>
            ...
            {!selectedFile ? (
              ...
              >
                ...
                <p className="text-sm font-medium text-stone-200">
                  {t('payment.report.dragDropText', 'Bấm để chọn ảnh hoặc kéo thả ảnh vào đây')}
                </p>
                <p className="text-xs text-stone-500 mt-1">
                  {t('payment.report.dragDropHint', 'Hỗ trợ JPG, PNG, WEBP. Tối đa 5MB.')}
                </p>
              </div>
            ) : (
              <div className="bg-stone-900/40 border border-stone-800 rounded-2xl p-4 flex items-center justify-between gap-4">
                  ...
                  <div className="min-w-0 text-left">
                    ...
                    {uploading ? (
                      <span className="text-[11px] text-amber-500 font-medium flex items-center gap-1 mt-1">
                        ...
                        {t('payment.report.uploadingStatus', 'Đang tải lên...')}
                      </span>
                    ) : isUploaded ? (
                      <span className="text-[11px] text-emerald-500 font-semibold flex items-center gap-1 mt-1">
                        ...
                        {t('payment.report.uploadedStatus', 'Đã tải lên')}
                      </span>
                    ) : null}
                  </div>
                </div>
                ...
                <button
                  ...
                  title={t('payment.report.deleteImage', 'Xóa ảnh')}
                >
        ...
        <div className="p-6 border-t border-stone-800/60 flex gap-4 flex-shrink-0">
          <button 
            ...
            disabled={isSubmitting || uploading}
          >
            {t('payment.report.cancelButton', 'Hủy Bỏ')}
          </button>
          <button 
            type="button"
            ...
            disabled={isSubmitting || uploading}
          >
            {isSubmitting ? t('payment.report.submittingText', 'Đang gửi...') : t('payment.report.submitButton', 'Gửi Báo Cáo')}
          </button>
        </div>
